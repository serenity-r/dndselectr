...
itemUI <- function(id, server=FALSE, session=getDefaultReactiveDomain()) {
  # Note: You might need to first wrap the id in session$ns() to account for
  #   embedded modules.  In other words, it is necessary to know where you are
  #   in the namespace stack.  For this particular example, it is not necessary.
  ns <- NS(session$ns(id))
...
shinyApp(
  ...
  server = function(input, output, session) {
    ...
    output$ui_dropzone <- renderUI({
      # Note: You must isolate the dropzone if you have presets with reactable elements!
      #  If you don't do this, then this will rerun every time the reactable elements
      #  in the preset change.
      isolate({
